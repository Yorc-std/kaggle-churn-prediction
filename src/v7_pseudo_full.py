"""
V7: V2 XGBoost + Pseudo Labels + Full Training
验证 Pseudo Labels + 充分训练的组合效果
"""
import pandas as pd
import numpy as np
...
def train_with_pseudo_labels(train_df, test_df, y_train, smoothing=3, confidence_threshold=0.95):
    """Pseudo Labels + 充分训练"""
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, y_train), 1):
        ...
        print(f"AUC: {roc_auc_score(y_val_fold, oof_preds[val_idx]):.6f} ({best_iteration} rounds)")
        sys.stdout.flush()
    ...
    # 第二轮：加入 Pseudo Labels + 充分训练
    print(f"\n{'='*60}")
    print(f"第 2 轮训练（加入 Pseudo Labels + 充分训练）")
    print(f"{'='*60}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, y_train), 1):
        ...
        best_iteration = model.best_iteration
        print(f"AUC: {roc_auc_score(y_val_fold, oof_preds_aug[val_idx]):.6f} ({best_iteration} rounds)")
        sys.stdout.flush()
...
# 训练
oof_auc, test_preds = train_with_pseudo_labels(train_fe, test_fe, y, smoothing=3, confidence_threshold=0.95)

print(f"\n{'='*60}")
print(f"最终结果")
print(f"{'='*60}")
print(f"OOF AUC: {oof_auc:.6f}")
print(f"V2 XGBoost + Pseudo (lr=0.05, n=1000): 0.916403")
...
submission.to_csv('submissions/v7_pseudo_full.csv', index=False)
print(f"\n提交文件已保存: submissions/v7_pseudo_full.csv")
